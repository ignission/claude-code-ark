...
文書は複数のセッションの Claude と人間が同じファイルを書き換える。読み手が知りたいのは
**どのブロックに人間が手を入れたか**の 1 点なので、`data-ark-id` を持つブロック要素へ
`data-ark-author="human"` を付けてそれを記す。本文は HTML が正準 source なので、書き手も
model ではなく HTML の属性に置く（model に書き手を複製しない）。

- **自分が書いたブロックに印は付けない**。`data-ark-author="claude"` は廃止した（文書の
  ほぼ全ブロックに付いて本文より目立ち、唯一読みたい差である「人間が触った箇所」を
  埋めてしまうため）。ボードにも Claude のバッジは出ない。既存文書に残っている
  `claude` はそのままでよい（サーバーは受理する）
- 値は `human` と `claude` の 2 つだけ。それ以外の値、`data-ark-id` の無い要素への付与、
  1 要素内の重複はサーバーが 422 で拒否する
- `data-ark-author="human"` は、人間がボード上で直接編集したブロックに編集層が自動で
  付ける（誤字を1つ直しただけでも付く）。Claude が自分でこの属性を付けてよいのは、
  人間がコメントや会話で下した決定を転記するときだけで、出典が分かる一文
  （「コメント #thread への回答より」等）を本文に添える
- 文書を読むときは、`data-ark-author="human"` が付いたブロックを人間が手を入れた本文
  として扱う。決定かどうかは属性ではなく本文の記述で判断する。誤字修正の1文字も
  `human` になるため、`human` を人間の決定の印だと読んではならない
- 編集層が `human` を自動で付けるのは、他の `data-ark-id` を内包しない最内側の葉
  ブロックだけ（文書全体を包む容器が丸ごと `human` になる事故を防ぐため）。容器ブロックに
  `human` が無くても、その中身を人間が書いていないとは限らない

ボードは `human` が付いたブロックにだけ「人間」のバッジを表示する。無印のブロックには
何も出ない。

```html
<p data-ark-id="s2-p1">推奨: B 案（同期 API）。理由は…</p>
...
`code` に背景色を当てるときは、それが `pre` の中の `<code>` にも効くことに注意する。
`pre` を濃色にした文書でこれをやると、明るい文字が明るい背景に重なってコードが
読めなくなる。ハーネスが `pre code` を既定で戻すので**書かなくてよい**が、
意図して `pre` の中の code を飾るなら `pre > code` 等、より詳細度の高い形で書く。
