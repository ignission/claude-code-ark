# flow-x plan 作成プロンプト (codex exec 用)

このファイルは `/flow-x` skill (P2 プラン phase および `--plan-only` モード) が **codex (`codex exec`)** に渡すプロンプトの雛形。supervisor は以下のプレースホルダを実際の値で置換してから渡す。
...
## codex への指示（このまま `codex exec` に渡す）

> flow-x では plan 立案を **codex** が担当する (通常 flow の `flow-plan-writer` subagent の代わり)。以下は codex exec に渡すプロンプト。

あなたは {{WORK_ID}} の実装 plan 作成を担当します。**plan 作成のみを行い、実装には進みません**（コード変更・git 状態変更は禁止）。許可される操作は以下のみ: `cd` / `ls` / `find` / `rg`・`grep` / `cat` / `sed` / `awk` 等の **読み取り系コマンド**による調査と、**plan ファイルへの Write** だけ。

**最優先**: plan を書く前に **プロジェクト規約を必ず読む** — プロジェクト直下の `CLAUDE.md`、`.claude/rules/backend-architecture.md` / `backend-testing.md` / `backend-migration.md` / `frontend-codegen.md`。plan の設計判断はこれら規約に準拠させること。一般論より既存規約・既存実装との一貫性を優先する。
...
- ❌ 他 skill 呼び出し
- ❌ `gh issue` 操作（GitHub Issue の取得・ラベル更新は supervisor が完了済み。本文は ISSUE_BODY に含まれている）
