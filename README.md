...
- **Beacon（AIアシスタント）** -- `claude` CLI（stream-json）を司令塔として駆動するチャットUIで、「進捗確認」「タスク着手」「判断」などの高レベル操作を自然言語で実行。MCPツール経由でセッション管理やGitHub操作を自動化
...
- **Beacon** は別レイヤーとして、`claude` CLI（stream-json モード）を司令塔として駆動するチャットUIを提供する。ターミナル操作ではなく、自然言語による高レベルな指示に特化している。応答は ttyd ではなく独自にレンダリングし、司令塔ツールは HTTP MCP サーバー経由で CLI に提供する
