...
summary_create_private_file() {
  ...
  if [ -e "$target" ] || [ -L "$target" ]; then
    ctx_validate_xdg_file "$target" || return 1
    : >"$target" || return 1
  else
...
summary_try_llm() {
  ...
  CTX_SUMMARIZE_LLM=0
  CTX_SUMMARIZE_MODEL=
  if [ -n "${CTX_CONFIG_FILE:-}" ]; then
    ctx_config_read_summarize >/dev/null 2>&1 || return 0
  fi
  [ "$CTX_SUMMARIZE_LLM" -eq 1 ] || return 0
  api_key=${ANTHROPIC_API_KEY:-}
  [ -n "$api_key" ] || return 0
  [ -n "$CTX_SUMMARIZE_MODEL" ] || return 0
  ...
  jq -n --arg model "$CTX_SUMMARIZE_MODEL" --rawfile mechanical "$summary" '
    {
      model:$model,
      max_tokens:512,
      temperature:0,
  ...
  ctx_validate_xdg_file "$SUMMARY_NEW" >/dev/null 2>&1 || return 0
...
"*|*""*|*"	"*) return 1 ;; esac
  ...
  if [ -e "$summary" ] || [ -L "$summary" ]; then ctx_validate_xdg_file "$summary" || return 1; fi
  ...
  ctx_validate_xdg_file "$SUMMARY_NEW" || return 1
