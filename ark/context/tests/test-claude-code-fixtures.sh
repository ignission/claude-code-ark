...
capture="$ROOT/ark/context/adapters/claude-code/tests/capture-post-tool-batch.sh"
tmp=$(mktemp -d)
tmp=$(cd "$tmp" && pwd -P)
...
failure_capture="$ROOT/ark/context/adapters/claude-code/tests/capture-post-tool-use-failure.sh"
printf 'old\n' >"$tmp/failure-out"
...
spec="$ROOT/docs/superpowers/specs/ark-context-implementation-spec.md"
for phrase in \
