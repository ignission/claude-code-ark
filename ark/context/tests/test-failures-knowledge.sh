...
FAILURES_LIB="$ROOT/ark/context/scripts/lib/failures-knowledge.sh"
TEST_TMP=$(mktemp -d)
...
session="$XDG_DATA_HOME/ark/context/sessions/33633633633633633633633633633633"
...
run_case ctx_knowledge_initialize "$session" "$knowledge"
...
assert_eq "session knowledge mode" 600 "$(ctx_stat "$session/knowledge/failures.md" | awk '{print $2}')"
...
run_case ctx_knowledge_initialize "$missing_session" "$missing_knowledge"
...
assert_eq "empty snapshot mode" 600 "$(ctx_stat "$missing_session/knowledge/failures.md" | awk '{print $2}')"
...
assert_eq "inbox mode" 600 "$(ctx_stat "$inbox" | awk '{print $2}')"
assert_eq "inbox candidate count" 2 "$(grep -Ec '^<!-- ark-context-candidate:[0-9a-f]{64} -->$' "$inbox")"
marker_separator=$(printf '\037')
bash_marker=$(ctx_sha256 "$sid${marker_separator}Bash${marker_separator}nonzero_exit${marker_separator}1${marker_separator}3")
read_marker=$(ctx_sha256 "$sid${marker_separator}Read${marker_separator}permission_denied${marker_separator}4${marker_separator}4")
grep -F -x "<!-- ark-context-candidate:$bash_marker -->" "$inbox" >/dev/null 2>&1 \
  || test_fail "inbox lacks the fixed Bash marker"
grep -F -x "<!-- ark-context-candidate:$read_marker -->" "$inbox" >/dev/null 2>&1 \
...
assert_eq "inbox publish failure preserves mode" 600 \
  "$(ctx_stat "$publish_failure_knowledge/failures-inbox.md" | awk '{print $2}')"
...
assert_eq "parallel append keeps two markers" 2 "$(grep -Ec '^<!-- ark-context-candidate:[0-9a-f]{64} -->$' "$parallel_inbox")"
...
empty_session="$XDG_DATA_HOME/ark/context/sessions/55555555555555555555555555555555"
...
find "$ROOT/ark/context/adapters" "$ROOT/ark/context/hooks" "$ROOT/ark/context/scripts" -type f -name '*.sh' -print \
...
grep -E '(cp|mv).*(failures-inbox\.md).*(failures\.md)' "$ROOT/ark/context"/scripts/*.sh \
  "$ROOT/ark/context"/scripts/lib/*.sh >/dev/null 2>&1
