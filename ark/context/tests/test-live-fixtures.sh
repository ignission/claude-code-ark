...
WRAPPER="$ROOT/ark/context/adapters/claude-code/post-tool-batch.sh"
FAILURE_WRAPPER="$ROOT/ark/context/adapters/claude-code/post-tool-use-failure.sh"
SINGLE="$ROOT/ark/context/adapters/claude-code/tests/fixtures/post-tool-batch-single-2.1.215.json"
PARALLEL="$ROOT/ark/context/adapters/claude-code/tests/fixtures/post-tool-batch-parallel-2.1.215.json"
TEST_TMP=$(mktemp -d)
...
failure_provenance=$(find "$ROOT/ark/context/adapters/claude-code/tests/fixtures" -type f -name 'post-tool-use-failure-provenance-*.txt')
...
for failure_case in bash-exit-7 mcp-error read-missing; do
  failure_fixture="$ROOT/ark/context/adapters/claude-code/tests/fixtures/post-tool-use-failure-$failure_case-$failure_version.json"
  i=1
  while [ "$i" -le 20 ]; do
    start=$(now_ns)
...
capture_parallel_fixture="$ROOT/ark/context/adapters/claude-code/tests/fixtures/post-tool-use-failure-bash-exit-7-$failure_version.json"
