...
FIXTURE="$ROOT/ark/context/adapters/claude-code/tests/fixtures/stop-incomplete.json"
TEST_TMP=$(mktemp -d)
...
assert_eq "one-shot flag mode" 600 "$(ctx_stat "$session/stop_once" | awk '{print $2}')"
...
complete_session="$XDG_DATA_HOME/ark/context/sessions/44444444444444444444444444444444"
...
mismatch_session="$XDG_DATA_HOME/ark/context/sessions/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
...
unsafe_session="$XDG_DATA_HOME/ark/context/sessions/55555555555555555555555555555555"
...
bad_flag_session="$XDG_DATA_HOME/ark/context/sessions/77777777777777777777777777777777"
...
ln -s "$complete_session" "$XDG_DATA_HOME/ark/context/sessions/symlink-session"
assert_quiet_stop "symlink session" env ARK_SESSION_DIR="$XDG_DATA_HOME/ark/context/sessions/symlink-session" \
  /bin/bash "$HOOK" <"$input"
...
jq --rawfile message "$large_message" \
  ...
  "$input" >"$large_input"
large_session="$XDG_DATA_HOME/ark/context/sessions/88888888888888888888888888888888"
...
lf_session="$XDG_DATA_HOME/ark/context/sessions/abababababababababababababababab"
...
parallel_session="$XDG_DATA_HOME/ark/context/sessions/66666666666666666666666666666666"
...
assert_eq "parallel one-shot flag mode" 600 "$(ctx_stat "$parallel_session/stop_once" | awk '{print $2}')"
