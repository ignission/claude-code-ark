...
SUMMARIZE="$ROOT/ark/context/scripts/summarize-errors.sh"
TEST_TMP=$(mktemp -d)
...
run_fallback_case() {
  ...
  if [ "$fallback_key" = absent ]; then
    run_case env -u ANTHROPIC_API_KEY PATH="$fallback_path" ARK_SESSION_DIR="$fallback_session" \
      CTX_CONFIG_FILE="$fallback_config" FAKE_CURL_MODE="$fallback_mode" /bin/bash "$SUMMARIZE"
  else
    run_case env PATH="$fallback_path" ARK_SESSION_DIR="$fallback_session" CTX_CONFIG_FILE="$fallback_config" \
...
run_case env PATH="$fakebin:$PATH" ARK_SESSION_DIR="$success_session" CTX_CONFIG_FILE="$success_config" \
...
: >"$curl_log"
run_case env PATH="$fakebin:$PATH" ARK_SESSION_DIR="$large_response_session" CTX_CONFIG_FILE="$large_response_config" \
