...
assert_eq "single increments once" 1 "$(cat "$cache/step_count")"
...
assert_eq "parallel batch increments once" 2 "$(cat "$cache/step_count")"
...
for input in "$TEST_TMP/invalid" "$TEST_TMP/array" "$TEST_TMP/other"; do
  ...
  assert_eq "invalid envelope has no side effect" "$before" "$(cat "$cache/step_count")"
...
assert_eq "oversized stdin has no side effect" 3 "$(cat "$cache/step_count")"
