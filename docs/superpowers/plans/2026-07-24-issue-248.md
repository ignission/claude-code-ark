...
依頼に記載された 2 ファイルはこの Ark worktree には存在せず、別リポジトリにあったため、参照専用で確認した。

- 外部リポジトリの実図 2 枚を、手動座標と境界を維持する後方互換、および可変高要素による overlap の検討材料とした。固有のドメイン要素や構造は fixture に持ち込まない。
- 実装 test は外部リポジトリのファイルを直接参照せず、レイアウト条件を一般化した Ark の E2E fixture で再現する。実図の座標除去や更新は別リポジトリの作業なので本 Issue の変更対象にしない。
...
- 有限な `ext.x/y` を持つ node は引き続き絶対優先で、group のために移動しない。手動座標を持つ既存の外部図は現行座標と境界をそのまま維持する。
...
- [ ] **Step 1: 外部実図の条件を一般化した coordinate-less fixture を作る（2〜5分）**

  固有のドメイン要素を使わず、複数の disjoint flat group と自由 node、group 内外の edge、可変 fields、異なる node width、group ごとに異なる境界 padding を持たせ、node の `ext.x/y` を省く。

- [ ] **Step 2: 全 node 非重複の Red を書く（2〜5分）**

  `graphNodeBoxes()` の全 pair を `expectNoOverlaps()` に通し、fields の内容に応じて node の実測 height が異なることも前提 assertion にする。
...
- 外部リポジトリの実図 2 枚の座標除去と、そのリポジトリへの反映は非ゴール。
...
- 外部リポジトリの実図 2 枚は Ark worktree にない。Ark 内 E2E fixture で一般化して再現する方針は確定しているが、外部リポジトリの実ファイルを coordinate-less へ移行する時期と担当は本 Issue 外で未確定。
