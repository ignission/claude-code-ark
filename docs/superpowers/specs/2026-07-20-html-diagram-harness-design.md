...
第一ユースケースは、コードベースを読んでドメインモデリングを図解し、設計を図の上で練って会話へ戻すことにある（具体例: サンプルプロジェクトの購買フロー設計）。
...
| 段階 | 内容 | 出口条件 |
| --- | --- | --- |
| B-0 | 新ハーネス、L2 生成規約 skill、`board_open`、サーバー側の差分生成 | サンプルプロジェクトの購買フローで受け入れシナリオが 1 周回る |
...
サンプルプロジェクトのセッションで「購買フローをドメインモデリングして図解して」と依頼する。
