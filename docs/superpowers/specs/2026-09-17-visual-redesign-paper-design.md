...
- **狭い幅では装飾から畳む**。1タップの操作は最後まで消さない
  - コンテナ42rem未満では、ブランチ・状態チップ・セグメント・「図」の**文言だけ**を `sr-only` に
    してアイコンにする。`display:none` にはしない (状態が支援技術から失われるため)。
    `StatusChip` と `SegmentedControl` は一覧・モバイルでも使うので、文言の span にだけ当たる
    `labelClassName` を呼び出し側から渡す形にし、渡さない呼び出しの見た目は変えない
  - それでも足りない幅は左のまとまり (主ラベル・ブランチ) が引き受ける。ブランチ → 主ラベルの順に
    縮み、最後は `overflow-hidden` で箱の中に収める。上部バーは横に溢れさせない
