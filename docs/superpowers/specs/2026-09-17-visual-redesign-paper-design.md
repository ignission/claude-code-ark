...
- **プロファイル (Linuxのみ)**
  - 表示: プロファイル未割り当て (今の「既定」) のときはチップを出さない。割り当てがあるときは、2行目の
    ...
    添える。プロファイル別の5色は使わない
  - 操作: 今はチップ自体が「このworktreeのプロファイル」を変えるボタンを兼ねている。チップを消しても
    操作は残すため、行のメニューに「プロファイル」の項目を置き、今の `RepoProfileMenu` の選択肢
    (このworktreeのプロファイル / リポジトリの設定を継承) をそこから開けるようにする
  - 「古い設定」(起動後にプロファイルの割り当てが変わった) の警告と「再起動」は、`--status-awaiting` の
    トーンで今のまま残す
- **行のメニュー** (右クリックと、ホバー時に出る `…` の両方で同じ中身):
  - 開く / 表示名を変更 / このセッションの通知オン・オフ / 再起動 / プロファイル
...
- **上部バーを1本に統合する**。今は、上部バー (端末/会話トグルと図) と、端末ペインの中のヘッダー
  ...
  - 右: 「図」の開閉ボタンと `…` メニュー
- `…` メニューの中身
  - 両モード共通: メッセージショートカット (送信と管理) / このセッションの通知オン・オフ / 削除
    (最下段、赤い文字)
  - 端末モードのときだけ: 端末のバッファをコピー / 画像を貼り付け / ファイルを添付 / 端末を再読み込み /
    入力バーを表示
  - 会話モードでの添付と画像の貼り付けは、会話の入力欄 (今もD&D・貼り付け・添付ボタンがある) が担う。
    端末側の添付確認画面は端末ペインの中にあり、会話モードでは親ごと隠れるため、メニューから呼ばない
...
- **ツール行を折りたたむ**
  ...
  - 実行中のまとまり (最後の `tool-call` の `status` が `running`) だけは、最新の1行を要約の下に出す
  - 1件だけのまとまりも同じく折りたたむ
...
- ヘッダー: 戻る + 主ラベル + ブランチ + 状態チップ。右に `…` (今の操作メニュー・メッセージショートカット・
  通知オン・オフ・削除を統合)
...
- **下部のバー**: 上段に「会話 / 端末 / 図」のセグメント、下段にモードごとの入力部
  ...
  - 会話モード: バーは浮かぶガラス (28px角丸)。下段は `SplitChatPane` の入力欄 (添付・入力・送信)。
    会話の本文はバーの下を通り、本文の下端にはバーの実測の高さ (ResizeObserver) + 12pxの余白を足す。
    入力欄が複数行に伸びても余白が追従する
