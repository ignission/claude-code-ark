...
- **上部バーを1本に統合する**。今は、上部バー (端末/会話トグルと図) と、端末ペインの中のヘッダー
  ...
  - 右: 1タップのボタン (ファイルを添付・画像を貼り付けは端末モードのときだけ、メッセージのショート
    カットは両モード) + 「図」の開閉ボタンと `…` メニュー
- `…` を開かずに届く1タップのボタン: ファイルを添付・画像を貼り付け (端末モードのときだけ) /
  メッセージのショートカット (両モード。送信の一覧と「ショートカットを管理」を1つのボタンから開く)
- `…` メニューの中身
  - 両モード共通: このセッションの通知オン・オフ / 削除 (最下段、赤い文字)
  - 端末モードのときだけ: 端末のバッファをコピー / 端末を再読み込み / 入力バーを表示
  - 会話モードでの添付と画像の貼り付けは、会話の入力欄 (今もD&D・貼り付け・添付ボタンがある) が担う。
    端末側の添付確認画面は端末ペインの中にあり、会話モードでは親ごと隠れるため、ボタンからもメニューからも呼ばない
...
- ヘッダー: 戻る + 主ラベル + ブランチ + 状態チップ。右に `…` (今の操作メニュー・通知オン・オフ・削除を統合。
  メッセージのショートカットは下部バーの1タップのボタンへ移した)
...
- **下部のバー**: 上段に「会話 / 端末 / 図」のセグメントと1タップの操作の行、下段にモードごとの入力部
  - 1タップの操作の行 (ファイルを添付・画像を貼り付け・メッセージのショートカット・スラッシュコマンド) は
    セグメントの下に置き、3モードのバーすべてに出す (`…` を開かずに届かせる)。画像を貼り付け・メッセージの
    ショートカット・スラッシュコマンドは3モードとも出すが、ファイルを添付だけは会話モードで出さない
    (会話の入力欄が自前の添付ボタンを持つため。端末・図モードは端末側の確認ダイアログの流儀で受ける)
