...
/** 設定の特定キーが期待値になるまで poll (debounce + I/O 完了待ちを wait 固定時間に頼らない) */
async function waitForSetting(
  ...
  throw new Error(
    `setting "${key}" did not become ${JSON.stringify(expected)} within ${timeoutMs}ms (last=${JSON.stringify(last)})`
  );
}
...
test("モバイル: リロードしても最後のタブが復元される (Beacon)", async ({
  ...
  // 設定が "beacon" として永続化されるまで poll (debounce 完了を待つ)
  await waitForSetting(request, "mobile.activeTab", "beacon");
...
test("モバイル: ボトムナビ操作自体は sessionSubView を破壊しない", async ({
  ...
  // beforeEach の初期値が settings に書き戻されるまで待つ
  await waitForSetting(request, "mobile.activeTab", "session");
  ...
  // 操作後: 最後にクリックしたのが session タブなので最終的に "session" に戻る
  await waitForSetting(request, "mobile.activeTab", "session");

  // sessionSubView は破壊されず初期値 "list" のままであることを確認
  const settings = await request.get("/api/settings").then(r => r.json());
