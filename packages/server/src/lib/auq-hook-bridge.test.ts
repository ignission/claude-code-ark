...
const EXPECTED_BOARD_CONTEXT = [
  ...
  'doc の本文ブロック（data-ark-id を持つ要素）には書き手を data-ark-author で記す。自分が書いた・書き換えたブロックには data-ark-author="claude" を付け、人間がコメントや会話で下した決定を転記するときだけ data-ark-author="human" を付ける。data-ark-author="human" は人間が手を入れた葉ブロックだけに付き（誤字修正でも付く、容器ブロックには付かない）、決定かどうかは本文の記述で判断する。',
].join("\n");
