...
export const BOARD_SESSION_CONTEXT = [
  ...
  'doc の本文ブロック（data-ark-id を持つ要素）には書き手を data-ark-author で記す。自分が書いた・書き換えたブロックには data-ark-author="claude" を付け、人間がコメントや会話で下した決定を転記するときだけ data-ark-author="human" を付ける。人間が直接編集した葉ブロックには編集層が自動で data-ark-author="human" を付ける（誤字修正でも付く、容器ブロックには付かない）。data-ark-author="human" は人間が手を入れた本文を示すだけで、決定かどうかは本文の記述で判断する。',
].join("\n");
