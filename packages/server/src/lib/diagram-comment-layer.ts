...
(function(){
  ...
  function collectTextNodes(anchor){
    ...
    var walker=document.createTreeWalker(anchor,NodeFilter.SHOW_TEXT,{
      acceptNode:function(node){
        ...
        if(parent.closest(".ark-comment-layer,.ark-author-badge,script,style,noscript"))return NodeFilter.FILTER_REJECT;
  ...
  // 人間が手を入れたブロックにだけバッジを置く。無印は何も出さない。
  // Claude のバッジは出さない: 文書のほぼ全ブロックに付いて本文より目立ち、
  // 「人間が触った箇所」という唯一読みたい差を埋めてしまうため（既存文書が持つ
  // data-ark-author="claude" は 422 にせず受理し、ただ表示しない）。
  var AUTHOR_ATTRIBUTE="data-ark-author";
  var AUTHOR_LABELS={human:"人間"};
  var AUTHOR_TITLES={human:"人間が手を入れた本文"};
  ...
  function authorBadgeTarget(anchor){
    ...
    return {parent:anchor,before:anchor.firstChild};
  }
  function buildAuthorBadge(anchor){
    var author=anchor.getAttribute(AUTHOR_ATTRIBUTE);
    if(!author||!Object.prototype.hasOwnProperty.call(AUTHOR_LABELS,author))return;
    var target=authorBadgeTarget(anchor);
    if(!target)return;
    var badge=element("span",AUTHOR_LABELS[author],"ark-author-badge");
  ...
  function buildAnchors(){
    ...
    document.querySelectorAll(selector).forEach(function(anchor){
      ...
      if(!graphMode)buildAuthorBadge(anchor);
      if(graphMode){
